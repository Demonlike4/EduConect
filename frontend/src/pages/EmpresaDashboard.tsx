...
const cleanText = (text: string) => {
    if (!text) return '';
    return text
        .replace(/Bakend \/ Fronted/g, 'Backend / Frontend')
        .replace(/\s*\(.*?\)\s*/g, '')
        .replace(/\s*-\s*.*?\s*\/\s*.*?\s*/g, '')
        .trim();
};
...
const EmpresaDashboard: React.FC = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const [offersRes, candidatesRes, tutorsRes, profileRes] = await Promise.all([
                    api.post('/empresa/ofertas', { email: user.email }),
                    api.post('/empresa/candidatos', { email: user.email }),
                    api.post('/empresa/tutores/list', { email: user.email }),
                    api.post('/empresa/profile', { email: user.email })
                ]);
    ...
    const handleProfileUpdate = async (e?: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            const res = await api.post('/empresa/profile/update', {
                ...profile
            });
    ...
    const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSubmitting(true);
        try {
            const res = await api.post('/empresa/profile/logo-upload', formData, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });
            if (res.data.logo && profile) {
                setProfile({ ...profile, logo: res.data.logo });
    ...
    const handleOfferImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSubmitting(true);
        try {
            const res = await api.post('/empresa/ofertas/upload-image', formData, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });
    ...
    const handleCreateOffer = async (e: React.FormEvent) => {
        ...
        try {
            if (editingOfertaId) {
                ...
                await api.post(`/empresa/ofertas/${editingOfertaId}/update`, {
                    email: user.email,
                    ...newOffer
                });
                ...
                // CREATE new offer
                await api.post('/empresa/ofertas/create', {
                    email: user.email,
                    ...newOffer
                });
            ...
            const offersRes = await api.post('/empresa/ofertas', { email: user?.email });
            setOfertas(offersRes.data.ofertas || []);
        } catch (error) {
            console.error(error);
    ...
    const handleCandidatoAction = async (id: number, status: string) => {
        ...
        try {
            await api.post('/empresa/candidatos/action', {
                email: user?.email,
                candidatura_id: id,
                action: status
            });
            ...
            // Refresh
            const candidatesRes = await api.post('/empresa/candidatos', { email: user?.email });
            setCandidatos(candidatesRes.data.candidatos || []);
    ...
    const handleAprobarTutor = async (tutorId: number) => {
        try {
            await api.post(`/empresa/tutores/${tutorId}/aprobar`, {
                email: user?.email
            });
    ...
    return (
        <DashboardLayout
                ...
                <>
                    ...
                    <div className="p-6 lg:p-8 border-b border-white/5 bg-zinc-950 relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-4">
                            <Logo variant="white" showTagline tagline="Portal Empresa" />
                        </div>
                    </div>
                    ...
                    <div className="p-6 lg:p-8 border-t border-white/5 bg-white/5 backdrop-blur-md">
                        <div className="flex items-center gap-3 p-1.5 pr-5 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 hover:border-white/10 transition-all cursor-pointer group">
                            {profile?.logo ? (
                                <CompanyLogo 
                                    logoPath={assetUrl(profile.logo)}
                                    companyName={empresaName}
                                    className="size-10 bg-linear-to-br from-indigo-500 to-purple-600 rounded-xl flex items-center justify-center text-white font-black text-sm shadow-inner overflow-hidden border border-white/10 group-hover:scale-105 transition-transform duration-500"
                                />
                            ) : (
            ...
            header={
                <header className="py-4 lg:py-0 h-auto lg:h-24 flex items-center justify-between px-6 lg:px-12 shrink-0 z-10 relative lg:sticky top-0" style={{ background: 'rgba(255,255,255,0.75)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.8)', boxShadow: '0 1px 20px rgba(99,102,241,0.06)' }}>
                    <div className="flex items-center gap-4">
                        <button
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className="relative" ref={notifRef}>
                            <button 
                                ...
                                className="size-10 bg-white border border-zinc-100 rounded-xl flex items-center justify-center text-zinc-500 hover:text-indigo-600 hover:shadow-xl hover:shadow-indigo-500/10 transition-all cursor-pointer relative"
                            >
                        ...
                        <div className="flex items-center gap-3 p-1 pr-4 bg-zinc-900 rounded-xl shadow-lg shadow-black/10 hover:scale-[1.02] transition-transform cursor-pointer">
                            {profile?.logo ? (
                                <CompanyLogo 
                                    logoPath={assetUrl(profile.logo)}
                                    companyName={empresaName}
                                    className="size-8 bg-linear-to-br from-indigo-500 to-purple-600 rounded-lg flex items-center justify-center text-white font-black text-xs shadow-inner overflow-hidden"
                                />
                            ) : (
        ...
        >
            ...
            <div className="flex-1 overflow-y-auto p-4 lg:p-8 space-y-8 custom-scrollbar">
                {/* Pending Tutor Alert Banner */}
                {pendingTutors.length > 0 && activeTab !== 'tutores' && (
                    <div className="animate-in slide-in-from-top-4 duration-500">
                        <div className="bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-200 dark:border-amber-900/30 rounded-[24px] p-6 flex items-center justify-between backdrop-blur-md">
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    <h4 className="text-sm font-black text-amber-900 dark:text-amber-400 uppercase tracking-tight">Solicitudes de Tutores Pendientes</h4>
                                    <p className="text-[10px] font-bold text-amber-700/70 dark:text-amber-500/70 uppercase tracking-widest mt-1">
                                        Hay {pendingTutors.length} {pendingTutors.length === 1 ? 'tutor que requiere' : 'tutores que requieren'} habilitación para gestionar alumnos.
                                    </p>
                                </div>
                            </div>
                            <button 
                                onClick={() => setActiveTab('tutores')}
                                className="px-6 h-11 bg-amber-500 hover:bg-amber-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-amber-500/20 flex items-center gap-2"
                            >
                                Gestionar Ahora
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-10 items-start">
                        <div className="xl:col-span-2 space-y-8 lg:space-y-10">
                            ...
                            <section className="rounded-[24px] overflow-hidden shadow-xl" style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.9)' }}>
                                <div className="px-8 py-5 border-b flex justify-between items-center sticky top-0 bg-white/80 backdrop-blur-md z-20" style={{ borderColor: 'rgba(0,0,0,0.05)' }}>
                                ...
                                <div className="p-0 overflow-auto max-h-[600px] custom-scrollbar relative">
                                    <table className="w-full text-left border-collapse">
                                        <thead className="sticky top-0 z-10 bg-zinc-50/95 backdrop-blur-xs">
                                            <tr className="text-[9px] lg:text-[10px] uppercase font-black text-zinc-400 tracking-widest border-b border-zinc-100 dark:border-zinc-800">
                                                    ...
                                                    <tr className="group hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-all border-b border-transparent hover:border-zinc-100 dark:hover:border-zinc-800">
                                                        <td className="px-8 py-5">
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                <div>
                                                                    <p className="text-sm font-bold text-zinc-900 dark:text-white leading-tight">{cleanText(c.nombre)}</p>
                                                                    <p className="text-[10px] text-zinc-400 font-medium mt-0.5">{c.email}</p>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td className="px-8 py-5">
                                                            <p className="text-[11px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-tight">{cleanText(c.puesto)}</p>
                                                        </td>
                                                        <td className="px-8 py-5">
                                                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-black tracking-widest uppercase ${c.estado === 'ADMITIDO' || c.estado === 'VALIDADO'
                                                                ? 'bg-emerald-100 text-emerald-700'
                                                                : 'bg-amber-100 text-amber-700'
                                                                }`}>
                                                                <span className={`size-1.5 rounded-full ${c.estado === 'ADMITIDO' || c.estado === 'VALIDADO' ? 'bg-emerald-500' : 'bg-amber-500'}`} />
                                                                {c.estado === 'ADMITIDO' || c.estado === 'VALIDADO' ? 'EN FORMACIÓN' : c.estado}
                                                            </span>
                                                        </td>
                        ...
                        <div className="xl:col-span-1 space-y-8 lg:sticky lg:top-10 max-h-fit lg:max-h-[calc(100vh-8rem)] overflow-y-auto custom-scrollbar pb-10">
                            ...
                            <section className="bg-white dark:bg-zinc-900 p-4 sm:p-6 lg:p-8 rounded-[32px] border border-zinc-200 dark:border-zinc-800 shadow-xl overflow-hidden">
                ...
                {activeTab === 'candidatos' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-10 pb-10">
                        <div className={`${selectedCandidato ? 'hidden lg:block' : 'block'} lg:col-span-1 bg-white dark:bg-zinc-900 rounded-[32px] border border-zinc-200 dark:border-zinc-800 shadow-xl overflow-hidden h-fit sticky top-24`}>
                                ...
                                {candidatos.map((c) => (
                                    ...
                                    >
                                        <div className="flex justify-between items-start">
                                            <p className={`text-sm font-black tracking-tight ${selectedCandidato?.id === c.id ? 'text-indigo-600' : 'text-zinc-900 dark:text-white'}`}>{cleanText(c.nombre)}</p>
                                            <span className={`px-2 py-0.5 rounded-lg text-[7px] font-semibold tracking-wide ${c.estado === 'VALIDADO' ? 'bg-emerald-100 text-emerald-700' :
                                                c.estado === 'POSTULADO' || c.estado === 'PENDIENTE_FIRMA_EMPRESA' ? 'bg-amber-100 text-amber-700' :
                                                    'bg-zinc-100 dark:bg-zinc-800 text-zinc-500'
                                                }`}>{c.estado === 'PENDIENTE_FIRMA_EMPRESA' ? 'Pte. Firma' : c.estado}</span>
                                        </div>
                                        <p className="text-[10px] font-bold text-zinc-400 mt-1 uppercase truncate">{cleanText(c.puesto)}</p>
                                    </button>
                                ))}
                                {candidatos.length === 0 && (
                        ...
                        <div className={`${selectedCandidato ? 'block' : 'hidden lg:block'} lg:col-span-3`}>
                            {selectedCandidato ? (
                                <div className="bg-white/90 dark:bg-zinc-900/90 backdrop-blur-2xl rounded-[40px] border border-white dark:border-zinc-800 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-500">
                                    {/* Botón de retorno para móviles en Master-Detail */}
                                    <div className="lg:hidden p-6 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/50">
                                        <button
                                            onClick={() => setSelectedCandidato(null)}
                                            className="flex items-center gap-2 text-zinc-500 dark:text-zinc-400 font-black text-[10px] uppercase tracking-[0.2em] hover:text-indigo-600 transition-colors"
                                        >
                                    ...
                                    <div className="p-10 border-b border-zinc-100 dark:border-zinc-800 relative overflow-hidden">
                                        <div className="absolute top-0 right-0 size-64 bg-gradient-to-br from-indigo-600 to-violet-600 opacity-5 rounded-full blur-3xl -mr-32 -mt-32"></div>
                                        <div className="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
                                            <div className="flex items-center gap-6">
                                                <div className="size-24 rounded-[32px] bg-gradient-to-br from-indigo-500 to-indigo-700 text-white flex items-center justify-center text-4xl font-black shadow-xl shadow-indigo-200/50 ring-4 ring-indigo-50 dark:ring-indigo-900/20 overflow-hidden">
                                                ...
                                                <div>
                                                    <h3 className="text-4xl font-black text-zinc-900 dark:text-white tracking-tighter">{cleanText(selectedCandidato.nombre)}</h3>
                                                    <div className="flex items-center gap-2 mt-2">
                                                        <p className="text-indigo-600 font-semibold tracking-wide text-[10px] tracking-widest">Postulación para {cleanText(selectedCandidato.puesto)}</p>
                                                        <span className="size-1 rounded-full bg-zinc-300"></span>
                                    ...
                                    {/* Biografía Profesional del Candidato */}
                                    <div className="px-10 py-8 bg-zinc-900 text-white border-b border-white/5 relative overflow-hidden group">
                                        <div className="absolute top-0 right-0 size-64 bg-indigo-500/10 rounded-full blur-3xl -mr-32 -mt-32 transition-all group-hover:bg-indigo-500/20"></div>
                                        <div className="relative z-10 flex items-start gap-6">
                                            <div className="size-12 bg-white/10 rounded-xl flex items-center justify-center text-indigo-400 shrink-0 border border-white/10">
                                                <span className="material-symbols-outlined text-2xl">person_book</span>
                                            </div>
                                            <div>
                                                <h4 className="text-[10px] font-black text-indigo-300/50 uppercase tracking-[0.3em] mb-2">Extracto Profesional</h4>
                                                <p className="text-base font-medium leading-relaxed italic text-zinc-300">
                                                    "{selectedCandidato.bio || 'Este candidato aún no ha completado su biografía profesional.'}"
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-10 space-y-8">
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 text-zinc-900 dark:text-white">
                                            <div className="lg:col-span-2 space-y-8">
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                    <section className="p-4 sm:p-6 lg:p-8 rounded-[32px] bg-zinc-50 dark:bg-zinc-800/30 border border-zinc-100 dark:border-zinc-800 group hover:border-indigo-600/20 transition-all">
                                                ...
                                                <section className="p-4 sm:p-6 lg:p-8 rounded-[32px] bg-slate-900 text-white shadow-2xl relative overflow-hidden group">
                                                    ...
                                                    <div className="relative z-10 flex flex-col h-full justify-between min-h-[140px]">
                                                        <div>
                                                            ...
                                                            <div className="flex items-center gap-3">
                                                                <div className={`size-3 rounded-full ${selectedCandidato.estado === 'VALIDADO' ? 'bg-emerald-500' : 'bg-amber-500 animate-pulse shadow-[0_0_12px_rgba(245,158,11,0.5)]'}`}></div>
                                                                <p className={`text-sm font-semibold tracking-wide leading-none ${selectedCandidato.estado === 'VALIDADO' ? 'text-emerald-400' : 'text-amber-400'}`}>
                                                                    {selectedCandidato.estado === 'PENDIENTE_FIRMA_EMPRESA' ? 'Pendiente de Firma' : selectedCandidato.estado}
                                                                </p>
                                                            </div>
                                                        </div>
                                            ...
                                            <div className="space-y-6">
                                                {selectedCandidato.cv ? (
                                                    <a
                                                        href={assetUrl(`/uploads/cv/${selectedCandidato.cv}`)}
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        className="p-4 sm:p-6 lg:p-8 rounded-[32px] border-2 border-dashed border-zinc-200 dark:border-zinc-800 flex flex-col items-center justify-center text-center group hover:bg-indigo-50 dark:hover:bg-indigo-900/10 hover:border-indigo-300 dark:hover:border-indigo-700 transition-all cursor-pointer aspect-square no-underline"
                                                    >
                                                        <div className="size-16 rounded-2xl bg-zinc-50 dark:bg-zinc-800 flex items-center justify-center text-zinc-400 group-hover:text-indigo-600 group-hover:bg-indigo-50 dark:group-hover:bg-indigo-900/30 transition-colors mb-4 ring-8 ring-zinc-50/50 dark:ring-zinc-800/50">
                                                    ...
                                                    <div className="p-4 sm:p-6 lg:p-8 rounded-[32px] border-2 border-dashed border-zinc-200 dark:border-zinc-800 flex flex-col items-center justify-center text-center aspect-square opacity-50">
                                                        <div className="size-16 rounded-2xl bg-zinc-50 dark:bg-zinc-800 flex items-center justify-center text-zinc-400 mb-4 ring-8 ring-zinc-50/50 dark:ring-zinc-800/50">
                                                            <span className="material-symbols-outlined text-4xl">contact_page</span>
                                                        </div>
                                                        <h4 className="text-sm font-black text-zinc-900 dark:text-white uppercase tracking-widest mb-1">Expediente CV</h4>
                                                        <p className="text-[10px] text-zinc-400 font-bold uppercase">No disponible</p>
                                                    </div>
                                                )}

                                                <div className="p-4 sm:p-6 lg:p-8 rounded-[32px] bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-800/50">
                        ...
                        <form onSubmit={handleCreateOffer} className="p-10 space-y-10">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-3">
                                    ...
                                    <input
                                        ...
                                        className="w-full h-14 bg-zinc-50 dark:bg-zinc-800 border-none rounded-2xl px-6 font-bold dark:text-white outline-none focus:ring-4 focus:ring-indigo-600/10 transition-all placeholder:text-slate-500"
                            ...
                            <div className="space-y-3">
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-zinc-50 dark:bg-zinc-800 border-none rounded-[32px] p-4 sm:p-6 lg:p-8 font-medium dark:text-white outline-none h-48 focus:ring-4 focus:ring-indigo-600/10 transition-all placeholder:text-slate-500"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-3">
                                    ...
                                    <input
                                        ...
                                        className="w-full h-14 bg-zinc-50 dark:bg-zinc-800 border-none rounded-2xl px-6 font-bold dark:text-white outline-none placeholder:text-slate-500"
                                ...
                                <div className="space-y-3">
                                    ...
                                    <input
                                        ...
                                        className="w-full h-14 bg-zinc-50 dark:bg-zinc-800 border-none rounded-2xl px-6 font-bold dark:text-white outline-none focus:ring-4 focus:ring-indigo-600/10 transition-all placeholder:text-slate-500"
                                        placeholder="Ej: Completa / Intensiva"
                                    />
                                </div>
                                <div className="space-y-3">
                        ...
                        <div className="bg-white dark:bg-zinc-900 rounded-[40px] border border-zinc-200 dark:border-zinc-800 shadow-xl overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700">
                            ...
                            <div className="p-10">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                    {activeTutors.map((t) => (
                                        <div key={t.id} className="p-4 sm:p-6 lg:p-8 bg-zinc-50 dark:bg-zinc-800/40 rounded-[32px] border border-zinc-100 dark:border-zinc-800 hover:border-indigo-600/30 transition-all group relative">
                        ...
                        <div className="max-w-6xl mx-auto space-y-10 pb-20 animate-in fade-in slide-in-from-bottom-5 duration-700">
                                        ...
                                        >
                                            {profile.logo ? (
                                                <CompanyLogo 
                                                    logoPath={assetUrl(profile.logo)}
                                                    companyName={empresaName}
                                                    className="w-full h-full flex items-center justify-center bg-zinc-50 dark:bg-zinc-800 text-zinc-200 font-light"
                                                />
                                            ) : (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-10">
                                ...
                                <div className="space-y-10">
                                    {/* Links y Contacto */}
                                    <div className="bg-white dark:bg-zinc-900 rounded-[32px] p-4 sm:p-6 lg:p-8 border border-zinc-200 dark:border-zinc-800 shadow-xl space-y-8">
                                    ...
                                    {isEditingProfile && (
                                        <div className="bg-zinc-950 border border-white/5 rounded-[32px] p-4 sm:p-6 lg:p-8 text-white space-y-8 animate-in zoom-in-95 duration-500 shadow-2xl">
                                            <div className="flex items-center gap-3">
                                    ...
                                    <div className="bg-white dark:bg-zinc-900 rounded-[32px] p-4 sm:p-6 lg:p-8 border border-zinc-100 dark:border-zinc-800 shadow-xl overflow-hidden relative">
                                        <h4 className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em] mb-8">Actividad Reciente</h4>
                                        <div className="space-y-6">
                                            <div className="flex items-center justify-between p-4 bg-indigo-50 dark:bg-indigo-500/10 rounded-2xl">
                                                <div className="flex items-center gap-3">
                                                    <div className="size-9 bg-indigo-600 rounded-xl flex items-center justify-center">
                                                        <span className="material-symbols-outlined text-white text-lg">work</span>
                                                    </div>
                                                    <div>
                                                        <p className="text-[10px] font-black text-zinc-500 dark:text-zinc-400 uppercase tracking-widest">Vacantes Activas</p>
                                                        <p className="text-2xl font-black text-zinc-900 dark:text-white leading-tight">{ofertas.filter(o => o.estado === 'Activa').length}</p>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="flex items-center justify-between p-4 bg-emerald-50 dark:bg-emerald-500/10 rounded-2xl">
                                                <div className="flex items-center gap-3">
                                                    <div className="size-9 bg-emerald-500 rounded-xl flex items-center justify-center">
                                                        <span className="material-symbols-outlined text-white text-lg">group</span>
                                                    </div>
                                                    <div>
                                                        <p className="text-[10px] font-black text-zinc-500 dark:text-zinc-400 uppercase tracking-widest">Candidaturas Recibidas</p>
                                                        <p className="text-2xl font-black text-zinc-900 dark:text-white leading-tight">{candidatos.length}</p>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="flex items-center justify-between p-4 bg-amber-50 dark:bg-amber-500/10 rounded-2xl">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="text-[10px] font-black text-zinc-500 dark:text-zinc-400 uppercase tracking-widest">En Prácticas</p>
                                                        <p className="text-2xl font-black text-zinc-900 dark:text-white leading-tight">{candidatos.filter(c => c.estado === 'VALIDADO').length}</p>
                                                    </div>
                                                </div>
                    ...
                    <div className="bg-white/95 dark:bg-zinc-900/95 backdrop-blur-2xl rounded-[40px] w-full max-w-xl shadow-[0_30px_100px_-20px_rgba(99,102,241,0.25)] overflow-hidden border border-white dark:border-zinc-800 animate-in zoom-in-95 duration-500 max-h-[90dvh] flex flex-col">
                        ...
                        <div className="p-6 sm:p-10 space-y-10 overflow-y-auto custom-scrollbar">
                            <section>
                                ...
                                <div className="relative group">
                                    <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                                        <span className="material-symbols-outlined text-indigo-400 group-focus-within:text-indigo-600 transition-colors">calendar_month</span>
                                    </div>
                                    <input
                                        type="text"
                                        value={actionData.tipoDuracion}
                                        onChange={(e) => setActionData({ ...actionData, tipoDuracion: e.target.value })}
                                        placeholder="Ej: 3 Meses / 380 horas"
                                        className="w-full pl-14 pr-5 py-5 bg-white dark:bg-zinc-900 border-2 border-zinc-100 dark:border-zinc-800 rounded-[24px] text-sm font-bold text-zinc-900 dark:text-white focus:outline-none focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/10 transition-all placeholder:text-zinc-400 placeholder:font-medium"
                                    />
                                    <div className="absolute inset-y-0 right-0 pr-5 flex items-center pointer-events-none">
                                        <span className="text-[10px] font-black text-zinc-300 uppercase tracking-widest">Periodo</span>
                                    </div>
                                </div>
                                <p className="text-[10px] text-zinc-400 mt-3 ml-2 font-medium leading-relaxed">
                                    Define el tiempo total de la FCT (ej. 3 meses) o la modalidad específica de la jornada.
                                </p>
                            </section>
                            ...
                            <section>
                                ...
                                <div className="relative group">
                                    <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                                        <span className="material-symbols-outlined text-indigo-400 group-focus-within:text-indigo-600 transition-colors">schedule</span>
                                    </div>
                                    <input
                                        type="text"
                                        value={actionData.horario}
                                        onChange={(e) => setActionData({ ...actionData, horario: e.target.value })}
                                        placeholder="Ej: 08:00 - 15:00 o Turno Rotativo"
                                        className="w-full pl-14 pr-5 py-5 bg-white dark:bg-zinc-900 border-2 border-zinc-100 dark:border-zinc-800 rounded-[24px] text-sm font-bold text-zinc-900 dark:text-white focus:outline-none focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/10 transition-all placeholder:text-zinc-400 placeholder:font-medium"
                                    />
                                    <div className="absolute inset-y-0 right-0 pr-5 flex items-center pointer-events-none">
                                        <span className="text-[10px] font-black text-zinc-300 uppercase tracking-widest">Horario</span>
                                    </div>
                                </div>
                                <p className="text-[10px] text-zinc-400 mt-3 ml-2 font-medium leading-relaxed">
                                    Especifica el horario exacto que deberá cumplir el alumno durante sus prácticas. Este será el horario oficial que se registrará en el convenio formativo.
                                </p>
                            </section>
                        </div>
