...
interface AlumnoAsignado {
    ...
    bio?: string;         // Biografía profesional del alumno
...
const TutorEmpresaDashboard: React.FC = () => {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            ...
            try {
                const response = await api.post('/tutor-empresa/dashboard');
    ...
    const handleOpenDetailModal = async (id: number) => {
        ...
        try {
            const response = await api.get(`/tutor/alumno/${id}`);
    ...
    const handleSign = async () => {
        ...
        try {
            await api.post(`/tutor-empresa/candidaturas/${candidaturaToSign.candidatura.id}/firmar`, {
    ...
    return (
        <DashboardLayout
                ...
                <>
                    ...
                    <div className="p-6 lg:p-8 border-b border-white/5 bg-zinc-950 relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-4">
                            <Logo variant="white" showTagline tagline="Tutor Empresa" />
                        </div>
                    </div>
            ...
            header={
                <header className="py-4 lg:py-0 h-auto lg:h-24 bg-white/70 backdrop-blur-2xl border-b border-zinc-100 flex items-center justify-between px-4 lg:px-12 relative lg:sticky top-0 z-20 transition-all">
                    ...
                    <div className="flex items-center gap-2 lg:gap-6 shrink-0">
                        ...
                        <div className="relative" ref={notifRef}>
                            ...
                            >
                                ...
                                {/* Bolita de alerta */}
                                <span className="absolute top-1.5 right-1.5 lg:top-2 lg:right-2 size-4 bg-gradient-to-r from-indigo-600 to-indigo-500 hover:scale-[1.02] hover:shadow-indigo-500/30 border-2 border-white rounded-full flex items-center justify-center text-[8px] font-black text-white">
                                    !
                                </span>
                            </button>
                            {/* Panel desplegable de notificaciones – visible en click */}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div className="bg-white dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                    <div className="flex justify-between items-start mb-4">
                                        <span className="material-symbols-outlined text-indigo-600 bg-indigo-50 p-3 rounded-2xl">person_check</span>
                                    </div>
                                        ...
                                        {alumnos.length === 0 ? (
                                            ...
                                            alumnos.slice(0, 3).map((al, idx) => (
                                                <div key={`${al.id}-${idx}`} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-slate-100 dark:border-slate-800">
                                                    <div className="flex items-center gap-4">
                                                        {/* Avatar del alumno */}
                                                        <div className="size-10 rounded-xl bg-indigo-100 dark:bg-indigo-500/20 flex items-center justify-center text-indigo-700 dark:text-indigo-300 font-black text-xs overflow-hidden">
                                                            {al.foto ? (
                                                                <img src={assetUrl(al.foto.startsWith('/uploads') ? al.foto : `/uploads/fotos/${al.foto}`)} className="w-full h-full object-cover" alt="Perfil" />
                                                            ) : (
                ...
                {activeTab === 'alumnos' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom duration-500">
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm overflow-hidden">
                            <div className="p-4 sm:p-6 lg:p-8 border-b border-slate-200 dark:border-slate-800">
                                            ...
                                            alumnos.map((row, idx) => (
                                                <tr key={`${row.id}-${idx}`} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                                                    ...
                                                    <td className="px-8 py-6">
                                                        <div className="flex items-center gap-4">
                                                            <div className="size-10 bg-indigo-100 dark:bg-indigo-500/20 rounded-xl flex items-center justify-center text-indigo-700 dark:text-indigo-300 text-xs font-black shadow-sm group-hover:scale-110 transition-transform overflow-hidden">
                                                                {row.foto ? (
                                                    ...
                                                    <td className="px-4 lg:px-8 py-6 text-right">
                                                        {row.candidatura.estado === 'PENDIENTE_FIRMA_EMPRESA' ? (
                                                            ...
                                                            <button
                                                                ...
                                                                className="text-white text-[9px] lg:text-xs font-semibold tracking-wide transition-all px-4 py-2 bg-gradient-to-r from-indigo-600 to-indigo-500 hover:scale-[1.02] hover:shadow-lg hover:shadow-indigo-500/30 rounded-xl"
                                                            >
                                                                Ver
                                                            </button>
                                                        )}
                            ...
                            <div className="sm:hidden p-4 space-y-4">
                                {loading ? (
                                    ...
                                    <div className="w-full flex flex-col items-center justify-center py-10 px-4 bg-slate-50/50 border-2 border-dashed border-slate-200 rounded-2xl">
                                        <span className="material-symbols-outlined text-4xl text-slate-300 mb-2">group</span>
                                        <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Sin alumnos</p>
                                    </div>
                                ) : (
                                    alumnos.map((row, idx) => (
                                        <div key={`${row.id}-${idx}`} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-5 space-y-5 shadow-sm">
                                            <div className="flex items-center gap-4">
                                                <div className="size-12 bg-indigo-50 dark:bg-indigo-500/10 rounded-2xl flex items-center justify-center text-sm font-black text-indigo-600 overflow-hidden shrink-0 border border-indigo-100 dark:border-indigo-500/20">
                                                    {row.foto ? (
                                                        <img src={assetUrl(row.foto.startsWith('/uploads') ? row.foto : `/uploads/fotos/${row.foto}`)} className="w-full h-full object-cover" alt="Perfil" />
                                                    ) : (
                                                        row.nombre.substring(0, 2).toUpperCase()
                                                ...
                                                <div className="min-w-0">
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <p className="text-sm font-black text-slate-900 dark:text-white truncate uppercase tracking-tighter leading-none">{row.nombre}</p>
                                                        <span className={`px-2 py-0.5 rounded-lg text-[7px] font-black uppercase tracking-widest ${row.candidatura.estado === 'VALIDADO' ? 'bg-emerald-500/10 text-emerald-600' : 'bg-amber-500/10 text-amber-600'}`}>
                                                            {row.candidatura.estado}
                                                        </span>
                                                    </div>
                                                    <p className="text-[10px] text-slate-400 font-bold truncate tracking-widest uppercase">{row.email}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-2xl border border-slate-100 dark:border-slate-800/50">
                                                    <p className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-1">Grado</p>
                                                    <p className="text-[10px] font-bold text-slate-700 dark:text-slate-200 truncate">{row.grado}</p>
                                                </div>
                                                <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-2xl border border-slate-100 dark:border-slate-800/50">
                                                    <p className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-1">Centro</p>
                                                    <p className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 truncate uppercase">{row.centro}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex gap-3">
                                                {row.candidatura.estado === 'PENDIENTE_FIRMA_EMPRESA' ? (
                                                    <button
                                                        onClick={() => handleOpenSignModal(row)}
                                                        className="flex-1 py-3.5 bg-emerald-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl shadow-emerald-500/20 flex items-center justify-center gap-2 active:scale-95 transition-all"
                                                    >
                                                    ...
                                                    <button
                                                        onClick={() => handleOpenDetailModal(row.id)}
                                                        className="flex-1 py-3.5 bg-gradient-to-r from-indigo-600 to-indigo-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl shadow-indigo-500/20 flex items-center justify-center gap-2 active:scale-95 transition-all"
                                                    >
                    ...
                    <div className="bg-white dark:bg-zinc-950 rounded-[40px] w-full max-w-6xl max-h-[95vh] shadow-2xl overflow-hidden border border-white/20 dark:border-zinc-800 flex flex-col animate-in slide-in-from-bottom-10 duration-500">
                        ...
                        {detailLoading ? (
                            ...
                            <>
                                ...
                                <div className="relative p-6 lg:p-12 overflow-hidden shrink-0 border-b border-zinc-100 dark:border-zinc-800/50 bg-gradient-to-r from-indigo-50/50 to-pink-50/50 dark:from-zinc-900 dark:to-zinc-900">
                                    {/* Decoraciones de fondo (blobs de color) */}
                                    <div className="absolute top-0 right-0 w-96 h-96 bg-gradient-to-r from-indigo-600 to-indigo-500 hover:scale-[1.02] hover:shadow-indigo-500/30/10 rounded-full -translate-y-1/2 translate-x-1/2 blur-[100px] animate-pulse opacity-20 dark:opacity-5"></div>
                                    ...
                                    {/* Botón de cierre absoluto en móvil, estático en desktop */}
                                    <button
                                        onClick={() => setIsDetailModalOpen(false)}
                                        className="absolute top-6 right-6 lg:top-10 lg:right-10 z-50 shrink-0 size-10 lg:size-14 rounded-2xl bg-white/80 backdrop-blur-md hover:bg-red-50 dark:hover:bg-red-500/10 flex items-center justify-center transition-all group hover:rotate-90 border border-zinc-100 dark:border-zinc-700/50 shadow-xl"
                                    >
                                        <span className="material-symbols-outlined text-zinc-400 group-hover:text-red-500 transition-colors text-xl sm:text-2xl">close</span>
                                    </button>

                                    <div className="relative flex flex-col sm:flex-row justify-between items-center sm:items-start gap-6 sm:gap-8 mt-4 sm:mt-0">
                                        <div className="flex flex-col sm:flex-row gap-6 sm:gap-10 items-center sm:items-start text-center sm:text-left min-w-0">
                                            ...
                                            <div className="relative group/photo shrink-0">
                                                ...
                                                <div className="relative size-24 sm:size-28 rounded-[2rem] bg-white dark:bg-zinc-800 flex items-center justify-center text-indigo-600 text-3xl sm:text-4xl font-black shadow-2xl border border-white/50 dark:border-zinc-700 overflow-hidden">
                                                    {studentDetail.foto ? (
                                                        <img src={assetUrl(studentDetail.foto.startsWith('/uploads') ? studentDetail.foto : `/uploads/fotos/${studentDetail.foto}`)} className="w-full h-full object-cover" alt="Perfil" />
                                                    ) : (
                                            ...
                                            <div className="min-w-0">
                                                <div className="flex flex-col sm:flex-row sm:items-center gap-2 lg:gap-4 mb-2 sm:mb-1 items-center">
                                                    <h3 className="text-xl sm:text-2xl lg:text-4xl font-black text-slate-900 dark:text-white tracking-tighter truncate uppercase leading-none max-w-[280px] sm:max-w-none">{studentDetail.nombre}</h3>
                                                    <span className="px-3 py-1 bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 text-[8px] sm:text-[10px] font-bold tracking-wide rounded-full border border-emerald-200 dark:border-emerald-500/20 shadow-sm whitespace-nowrap">
                                                        ACTIVO 2026/27
                                                    </span>
                                                </div>
                                                <p className="text-indigo-600/80 dark:text-indigo-400 font-black text-[10px] sm:text-sm lg:text-base uppercase tracking-[0.2em] mb-4 truncate max-w-[260px] sm:max-w-none mx-auto sm:mx-0">{studentDetail.grado}</p>
                                                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 lg:gap-6 text-zinc-500 dark:text-zinc-400 text-[9px] sm:text-xs">
                                                    {/* Email del alumno */}
                                                    <div className="flex items-center gap-2 bg-white/50 dark:bg-zinc-800/50 px-3 py-1.5 rounded-xl border border-slate-100 dark:border-zinc-700/50 sm:bg-transparent sm:p-0 sm:border-0">
                                                        <span className="material-symbols-outlined text-[14px] lg:text-[18px] opacity-70 text-indigo-500">mail</span>
                                                        <span className="font-bold tracking-tight">{studentDetail.email}</span>
                                                    </div>
                                                    {/* Indicador de usuario verificado */}
                                                    <div className="hidden min-[450px]:flex items-center gap-2 bg-indigo-50/50 dark:bg-indigo-500/5 px-3 py-1.5 rounded-xl border border-indigo-100/50 dark:border-indigo-500/10 sm:bg-transparent sm:p-0 sm:border-0">
                                                        <span className="material-symbols-outlined text-[14px] lg:text-[18px] text-indigo-500 font-bold">verified_user</span>
                                    ...
                                    {modalTab === 'info' ? (
                                        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 lg:gap-10 animate-in fade-in slide-in-from-bottom-6 duration-700 mt-2">
                                            ...
                                            <div className="md:col-span-5 flex flex-col gap-6 lg:gap-8">
                                                ...
                                                {/* Sección: Biografía Profesional */}
                                                <section className="bg-zinc-900 text-white p-6 lg:p-8 rounded-[32px] shadow-2xl relative overflow-hidden group border border-white/5">
                                                    <div className="absolute top-0 right-0 size-32 bg-indigo-500/10 rounded-full blur-3xl group-hover:bg-indigo-500/20 transition-all"></div>
                                                    <div className="flex items-center gap-3 mb-4 relative z-10">
                                                        <div className="size-9 bg-white/10 rounded-xl flex items-center justify-center text-indigo-400">
                                                            <span className="material-symbols-outlined text-[18px]">person_book</span>
                                                        </div>
                                                        <h4 className="font-black text-indigo-300/50 text-[10px] uppercase tracking-[0.25em]">Extracto Profesional</h4>
                                                    </div>
                                                    <p className="text-[13px] font-medium leading-relaxed italic text-zinc-300 relative z-10 px-1">
                                                        "{studentDetail.bio || 'Este alumno aún no ha redactado su biografía profesional.'}"
                                                    </p>
                                                </section>
                                                ...
                                                <section className="bg-white dark:bg-zinc-900 rounded-[32px] border border-zinc-200 dark:border-zinc-800 p-4 sm:p-6 lg:p-8 space-y-6 shadow-sm overflow-hidden relative">
                                                    ...
                                                    {studentDetail.candidatura?.estado === 'VALIDADO' ? (
                                                        <>
                                                            ...
                                                            <div className="relative">
                                                                ...
                                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 items-center bg-white/5 p-4 sm:p-6 lg:p-8 rounded-[32px] border border-white/10">
                                                                    <div className="space-y-2">
                                ...
                                <div className="p-4 sm:p-6 lg:p-8 border-t border-zinc-100 dark:border-zinc-800/50 bg-white dark:bg-zinc-950 flex flex-col sm:flex-row justify-between items-center gap-4 shrink-0">
            ...
            {isValModalOpen && candidaturaToSign && (
                <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] w-full max-w-lg shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-400">
                        ...
                        <div className="p-4 sm:p-6 lg:p-8 space-y-6">
                            {valStep === 1 ? (
                                <>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 p-4 sm:p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-100 dark:border-slate-800">
                        ...
                        <div className="p-4 sm:p-6 lg:p-8 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/20 flex flex-col sm:flex-row gap-3">
